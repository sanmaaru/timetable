...
class AuthTokenService:
    ...
    async def issue_refresh(owner_id: ULID, session: AsyncSession, expired_after: timedelta | None = None):
        ...
        await crud_refresh_token.create(refresh_token, session)
    ...
    async def reissue_refresh(
        ...
        await crud_refresh_token.delete(session, condition=[RefreshToken.owner_id == owner_id])
...
class AuthService:
    ...
    async def signup(
        ...
        token = await crud_identify_token.query(session, condition=[IdentifyToken.token_id == identify_token])
        if token is None:
        ...
        await crud_user.create(user, session)
    ...
    async def login(
        ...
        user = await crud_user.query(session, condition=[User.username == username])
    ...
    async def refresh(
        ...
        refresh_token = await crud_refresh_token.query(session, condition=[RefreshToken.token_id == refresh])
        if refresh_token is None:
            raise AuthorizationError(message='Invalid refresh token', payload={'invalid': 'refresh_token'})

        user_id = refresh_token.owner_id
        new_refresh = await AuthTokenService.reissue_refresh(user_id, refresh_token, session)
